const BlackJack = function(targetId) {
  ...
  function calcHandScore(whichHand) {
    ...
    // update hand's score
    whichHandTotal = whichHandValues.reduce((acc, next) => acc + next);
  ...
  const dealCard = whichHand => {
    ...
    with (cards[cardsDealt].style) {
      zIndex = "1000";
      top = cards[cardsDealt].fromtop + "px";
      left = cards[cardsDealt].fromleft + "px";
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (i < 10) {
      ...
      newChip.style.backgroundImage = `url(blue-chip.png)`;
      ...
      newChip.style.left = 100 + 100 * Math.floor(i / 10) + "px";
      newChip.style.backgroundImage = `url(red-chip.png)`;
      felt.appendChild(newChip);
    } else {
      // green 500 chips
      let newChip = chip.cloneNode(true);
      ...
      newChip.style.left = 100 + 100 * Math.floor(i / 10) + "px";
      newChip.style.backgroundImage = `url(green_chip.png)`;
      felt.appendChild(newChip);
    }
  }
